-- ==============================================================================
-- AURORA HUB - FLUENT UI VERSION (FULL LOGIC INTEGRATION)
-- ==============================================================================
...
local Tabs = {
    Main = Window:AddTab({ Title = "Main Farming", Icon = "swords" }),
    Stats = Window:AddTab({ Title = "Auto Stats", Icon = "bar-chart" }),
    Boss = Window:AddTab({ Title = "Boss & Events", Icon = "skull" }),
    Misc = Window:AddTab({ Title = "Misc Features", Icon = "box" })
}

-- ================== TAB MAIN ==================
Tabs.Main:AddSection("Farming Settings [ Attack Mode ]")
-- ==========================================
-- 1. VARIABEL & FUNGSI FAST ATTACK (TARUH DI ATAS)
-- ==========================================
...
-- ==========================================
-- 2. UI FLUENT KAMU (TARUH DI BAWAHNYA)
-- ==========================================
-- (Abaikan kode loadstring dan window creation sebelumnya, langsung fokus ke bagian Tab Main)

-- Dropdown untuk memilih mode kecepatan Fast Attack
local AttackSpeedDropdown = Tabs.Main:AddDropdown("AttackSpeed", {
    Title = "Attack Speed",
    Values = {"Normal Attack", "Fast Attack", "Super Attack", "Aurora Attack", "Super Rora Attack"},
    Multi = false,
    Default = 3 -- Default ke Super Attack
})

AttackSpeedDropdown:OnChanged(function(Value)
    _G.Settings.FastAttackMode = Value
    -- Jika toggle sedang aktif, restart loop dengan kecepatan baru
    if _G.Settings.FastAttack then
        StartAttacksWithMode(Value)
    end
end)

local WpDropdown = Tabs.Main:AddDropdown("FarmWp", { Title = "Select Farm Weapon", Values = {"Melee", "Blox Fruit", "Sword", "Gun"}, Multi = false, Default = 1 })
WpDropdown:OnChanged(function(Value) _G.ChooseWP = Value end)

local ModeDropdown = Tabs.Main:AddDropdown("FarmMode", { Title = "Select Farm Mode", Values = {"Level", "Bone", "Cake Prince", "Tyrant Of The Skies"}, Multi = false, Default = 1 })
ModeDropdown:OnChanged(function(Value) _G.SelectedFarmMode = Value end)
Tabs.Main:AddSection("Attack Feature")
local FarmToggle = Tabs.Main:AddToggle("StartFarmToggle", {Title = "Start Auto Farm", Default = false })
FarmToggle:OnChanged(function(Value) _G.StartFarm = Value end)

-- Toggle utama untuk menyalakan/mematikan Auto Click & Fast Attack
local Fast2Toggle = Tabs.Main:AddToggle("FastToggle", {Title = "Enable Fast Attack", Default = false })

Fast2Toggle:OnChanged(function(Value)
...
local KenToggle = Tabs.Main:AddToggle("KenToggle", {Title = "Accept Quest", Default = true })
KenToggle:OnChanged(function(Value) _G.AcceptQuest = Value end)

local BringToggle = Tabs.Main:AddToggle("BringToggle", {Title = "Bring Mob", Default = true })
BringToggle:OnChanged(function(Value) _B = Value end)

local nearfarm = Tabs.Main:AddToggle("Near toggle", {Title = "Farm Nearest Mob",  Default = false})
nearfarm:OnChanged(function(Value) _G.AutoFarmNear = Value end)
Tabs.Misc:AddSection("Settings Feature")
local Haki = Tabs.Misc:AddToggle("Haki", {Title = "Auto Haki", Description = "Hai Black Nigga", Default = true})
Haki:OnChanged(function(I)
Boud = I 
spawn(function()
	while wait(Sec) do
		pcall(function()
...
Window:SelectTab(1)
Fluent:Notify({ Title = "Aurora Hub", Content = "Semua Fungsi & Logic berhasil dimuat!", Duration = 5 })

-- ==============================================================================
-- 6. MAIN EXECUTION LOOPS (BACKGROUND)
-- ==============================================================================
-- ==============================================================================
-- STRUKTUR PENUTUP (Copy & Paste bagian ini ke paling bawah script kamu)
-- ==============================================================================
...
task.spawn(function()
    while task.wait(0.5) do
        if _G.StartFarm then
            _G.Level = (_G.SelectedFarmMode == "Level")
            _G.AutoFarm_Bone = (_G.SelectedFarmMode == "Bone")
            _G.AutoFarm_Cake = (_G.SelectedFarmMode == "Cake Prince")
            _G.AutoTyrant = (_G.SelectedFarmMode == "Tyrant Of The Skies")
        else
            _G.Level = false
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- ⚔️ 3. Atacar & Bring Mob
            if ClosestEnemy and ClosestEnemy:FindFirstChild("HumanoidRootPart") and (not _G.MaxFarmDistance or ShortestDistance <= _G.MaxFarmDistance) then
                
                -- Ambil target posisi dari musuh yang sedang diserang
                local TargetMobRoot = ClosestEnemy.HumanoidRootPart
                
                repeat
                    task.wait()
                    
                    -- ==========================================
                    -- 🧲 LOGIKA BRING MOB (DIMULAI)
                    -- ==========================================
                    if _B and workspace:FindFirstChild("Enemies") then
                        -- Perluas Simulation Radius
                        pcall(function()
                        ...
                        local bringRange = _G.BringRange or 300

                        for _, mob in pairs(workspace.Enemies:GetChildren()) do
                            local mobHum = mob:FindFirstChild("Humanoid")
                            local mobRoot = mob:FindFirstChild("HumanoidRootPart")

                            -- Cek musuh valid dan bukan target utama
                            if mob ~= ClosestEnemy and mobHum and mobRoot and mobHum.Health > 0 then
                                local distToTarget = (mobRoot.Position - TargetMobRoot.Position).Magnitude

                                -- Tarik musuh jika masuk jangkauan
                                if distToTarget <= bringRange then
                                    -- Matikan collision agar tidak bertumpuk keras & lag
                                    ...
                                    -- Nol-kan kecepatan agar tidak fling/melayang
                                    mobRoot.AssemblyLinearVelocity = Vector3.zero
                                    mobRoot.AssemblyAngularVelocity = Vector3.zero
                                    
                                    -- Set posisi langsung ke musuh utama (Magnet)
                                    mobRoot.CFrame = TargetMobRoot.CFrame
                                end
                            end
                        end
                    end
                    -- ==========================================
                    -- 🧲 LOGIKA BRING MOB (BERAKHIR)
                    -- ==========================================
