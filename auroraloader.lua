...
local function safeRequest(options)
    ...
    if ok and res then return res else return nil, tostring(res) end
end
...
local function ShowAuroraNotification(title, desc)
    ...
    NFrame.Position = UDim2.new(1, 20, 1, -90)
...
if key == "" then
    ShowAuroraNotification("❌ Aurora Hub", "Masukkan key dulu!")
    error("[ ❄ Aurora Hub ❄ ] Masukkan key dulu! Contoh: getgenv().key = 'AURORA-XXX'")
    return
end

ShowAuroraNotification("⏳ Aurora Hub", "Memverifikasi key...")
...
if response and response.StatusCode == 200 then
    ...
    if decoded.success and decoded.valid then
        ...
        local msg = decoded.message or "Key tidak valid"
        if string.find(msg, "HWID") then
            ShowAuroraNotification("🔒 Aurora Hub", "HWID Mismatch!")
            task.wait(1)
            LocalPlayer:Kick("[ ❄ Aurora Hub ❄ ]\n🔒 HWID Mismatch!")
        elseif string.find(msg, "Redeem") then
            ShowAuroraNotification("⚠️ Aurora Hub", "Redeem key di Discord dulu!")
            task.wait(1)
            LocalPlayer:Kick("[ ❄ Aurora Hub ❄ ]\n⚠️ Redeem key di Discord!")
        else
            ShowAuroraNotification("❌ Aurora Hub", msg)
            error("[ ❄ Aurora Hub ❄ ] " .. msg)
        end
    end
else
    ShowAuroraNotification("❌ Aurora Hub", "Gagal konek ke server!")
    error("[ ❄ Aurora Hub ❄ ] Server Error: " .. tostring(err))
end
