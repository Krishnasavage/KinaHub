local Config = {
	...
	ServiceId = 25850, -- ID Layanan PlatoBoost Anda
	...
	MainGuiName = "", -- Nama GUI skrip utama untuk mengecek apakah sudah berjalan
...
-------------------------------------------------------------------------------
--! NEW AURORAHUB GUI CREATION (IMPROVED UI & ANIMATION)
-------------------------------------------------------------------------------
...
local function CreateGUI()
	...
	if pGui:FindFirstChild("AuroraHub") then
		pGui.AuroraHub:Destroy()
	end

	--! Fungsi Animasi Warna Aurora
	local function AnimateAurora(label)
		local connection
		connection = RunService.RenderStepped:Connect(function()
			-- Mematikan loop secara otomatis jika UI ditutup / dihancurkan
			if not label or not label.Parent then
				connection:Disconnect()
				return
			end
			-- Logika matematika menggunakan 'sin' untuk efek transisi bolak-balik yang sangat mulus
			-- Membatasi warna di area Hijau, Biru Terang (Cyan), hingga Ungu (Pinkish)
			local hue = 0.6 + (math.sin(tick() * 1.5) * 0.25)
	...
	MainFrame.Size = UDim2.new(0, 0, 0, 0) -- Mulai dari size 0 untuk animasi Pop-in
	...
	-- Memanggil fungsi animasi untuk membuat Judul berwarna Aurora
	AnimateAurora(Title)
	...
	InputStroke.Color = Color3.fromRGB(45, 50, 60)
	...
	Input.FocusLost:Connect(function()
		TweenService:Create(InputStroke, TweenInfo.new(0.3), { Color = Color3.fromRGB(45, 50, 60) }):Play()
	end)
	...
	local function AnimateButton(button, normalColor, hoverColor)
		button.MouseEnter:Connect(function()
			...
			TweenService:Create(button.UIScale, TweenInfo.new(0.2), { Scale = 1.03 }):Play()
		end)
		button.MouseLeave:Connect(function()
			TweenService:Create(button, TweenInfo.new(0.2), { BackgroundColor3 = normalColor }):Play()
			TweenService:Create(button.UIScale, TweenInfo.new(0.2), { Scale = 1 }):Play()
		end)
		button.MouseButton1Down:Connect(function()
			TweenService:Create(button.UIScale, TweenInfo.new(0.1), { Scale = 0.95 }):Play()
		end)
		button.MouseButton1Up:Connect(function()
			TweenService:Create(button.UIScale, TweenInfo.new(0.1), { Scale = 1.03 }):Play()
		end)
	end
	...
	Verify.Size = UDim2.new(0.4, 0, 0, 42)
	Verify.Position = UDim2.new(0.075, 0, 0, 145)
	Verify.Text = "VERIFY KEY"
	Verify.Font = Enum.Font.GothamBold
	Verify.TextSize = 13
	Verify.TextColor3 = Color3.new(1, 1, 1)
	Verify.BackgroundColor3 = Color3.fromRGB(35, 100, 220)
	Verify.AutoButtonColor = false

	local VCorner = Instance.new("UICorner", Verify)
	VCorner.CornerRadius = UDim.new(0, 8)
	Instance.new("UIScale", Verify)
	AnimateButton(Verify, Color3.fromRGB(35, 100, 220), Color3.fromRGB(55, 130, 255))

	-- Portal Button
	local Portal = Instance.new("TextButton")
	Portal.Parent = MainFrame
	Portal.Size = UDim2.new(0.4, 0, 0, 42)
	Portal.Position = UDim2.new(0.525, 0, 0, 145)
	Portal.Text = "GET KEY"
	Portal.Font = Enum.Font.GothamBold
	Portal.TextSize = 13
	Portal.TextColor3 = Color3.new(1, 1, 1)
	Portal.BackgroundColor3 = Color3.fromRGB(40, 45, 55)
	Portal.AutoButtonColor = false

	local PCorner = Instance.new("UICorner", Portal)
	PCorner.CornerRadius = UDim.new(0, 8)
	Instance.new("UIScale", Portal)
	AnimateButton(Portal, Color3.fromRGB(40, 45, 55), Color3.fromRGB(60, 65, 80))
	...
	-- Developer Krizz Label
	local DevText = Instance.new("TextLabel")
	DevText.Parent = MainFrame
	DevText.Size = UDim2.new(1, 0, 0, 15)
	DevText.Position = UDim2.new(0, 0, 1, -35)
	DevText.BackgroundTransparency = 1
	DevText.Text = "Developed By Krizz"
	DevText.Font = Enum.Font.GothamBold
	DevText.TextSize = 12

	-- Memanggil fungsi animasi untuk tulisan "Developed By Krizz"
	AnimateAurora(DevText)
	...
	Footer.Text = "Protected by PlatoBoost"
	Footer.Font = Enum.Font.Gotham
	Footer.TextSize = 10
	Footer.TextColor3 = Color3.fromRGB(90, 90, 90)

	---------------------------------------------------------------------------
	--! INTERACTION & INTEGRATION LOGIC
	---------------------------------------------------------------------------
	...
	Verify.MouseButton1Click:Connect(function()
		...
		if success then
			Status.Text = "Success! Launching Hub..."
	...
	Portal.MouseButton1Click:Connect(function()
		Status.Text = "Generating unique link..."
		Status.TextColor3 = Color3.fromRGB(255, 200, 80)
		local success, link = cacheLink()
		if success then
			fSetClipboard(link)
			Status.Text = "Link copied to clipboard!"
			Status.TextColor3 = Color3.fromRGB(88, 166, 255)
	...
	if isfile and isfile(Config.KeyFileName) then
		...
		if savedKey ~= "" then
			Status.Text = "Saved key found, authenticating..."
			...
			task.spawn(function()
				...
				if success then
					...
					TweenService
						:Create(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
							Size = UDim2.new(0, 0, 0, 0),
							Position = UDim2.new(0.5, 0, 0.5, 0),
						})
						:Play()
					...
					Status.Text = "Saved key expired. Please get a new one."
